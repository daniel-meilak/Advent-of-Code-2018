...
struct unit{
   ...
   // constructors
   unit(int attack, char type, point pos): hp(200), attack(attack), type(type), pos(pos){};
   unit(const unit &unit): hp(unit.hp), attack(unit.attack), type(unit.type), pos(unit.pos){};
...
struct distance_func{
   ...
   distance_func(const point &p): p(p){};
...
std::vector<point> find_neighbours(const point &current, const std::vector<std::string> &grid){
   ...
   for (const point &dir : dirs){
      ...
      if (grid[n.y][n.x]=='.'){ output.push_back(n); }
   }
